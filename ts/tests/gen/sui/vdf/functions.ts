...
export function hashToInput(
  ...
  return tx.moveCall({
    target: `${getPublishedAt('sui', options?.env)}::vdf::hash_to_input`,
...
export function hashToInputInternal(
  ...
  return tx.moveCall({
    target: `${getPublishedAt('sui', options?.env)}::vdf::hash_to_input_internal`,
    arguments: [pure(tx, message, `vector<u8>`)],
  })
}
...
export function vdfVerify(
  ...
  return tx.moveCall({
    target: `${getPublishedAt('sui', options?.env)}::vdf::vdf_verify`,
    arguments: [
...
export function vdfVerifyInternal(
  ...
  return tx.moveCall({
    target: `${getPublishedAt('sui', options?.env)}::vdf::vdf_verify_internal`,
    arguments: [
      pure(tx, args.input, `vector<u8>`),
      pure(tx, args.output, `vector<u8>`),
      pure(tx, args.proof, `vector<u8>`),
